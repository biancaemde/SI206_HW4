...
class CardTests(unittest.TestCase):
	#test for if you create a card with rank 12, its rank will be Queen
	def test_cardrank(self):
		c = Card(rank=12)
		self.assertEqual(c.rank,'Queen')
	#test for if you make card with rank 1, it will be rank Ace
	def test_cardrank1(self):
		c = Card(rank=1)
		self.assertEqual(c.rank,'Ace')
	#test for if you create a card instance with rank 3, it will have rank 3
	def test_cardrank2(self):
		c = Card(rank=3)
		self.assertEqual(c.rank,3)
	#that for if you create card instance of wuit 1, it will be a Club
	def test_cardsuit(self):
		c = Card(suit=1)
		self.assertEqual(c.suit,'Clubs')
	#test that card instance with suit 2, will be suit Hearts
	def test_cardsuit1(self):
		c = Card(suit=2)
		self.assertEqual(c.suit,'Hearts')
	#test if you create card instance, it will have access to variable
	#suit_names that contains list ['Diamonds','Clubs','Hearts','Spades']
	def test_cardInstance(self):
		c = Card()
		self.assertEqual(c.suit_names,['Diamonds','Clubs','Hearts','Spades'])
	#test if you invoke __str__ method of card instance that was created
	#with suit=2 and rank=7, it returns strong "7 of Hearts"
	def test_cardInstance2(self):
		c = Card(suit=2, rank=7)
		self.assertEqual(str(c),'7 of Hearts')
	#test if you create deck instance, it will have 52 cards in cards instance variable
	def test_deckInstance(self):
		d = Deck()
		self.assertEqual(len(d.cards),52)
	#test if you invoke pop_card method on a deck, it will return a card instance
	def test_popInstance(self):
		d = Deck()
		c = Card()
		self.assertEqual(type(d.pop_card()), type(c))
	#test that the return value of the pay_war_game function is a tuple with 3
	#elements, first of which is a string
